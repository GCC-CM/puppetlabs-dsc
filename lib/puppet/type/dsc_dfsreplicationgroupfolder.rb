...
Puppet::Type.newtype(:dsc_dfsreplicationgroupfolder) do
  ...
  @doc = %q{
    ...
    'DFSDsc/DSCResources/MSFT_DFSReplicationGroupFolder/MSFT_DFSReplicationGroupFolder.schema.mof'
  ...
  def dscmeta_resource_friendly_name; 'DFSReplicationGroupFolder' end
  def dscmeta_resource_name; 'MSFT_DFSReplicationGroupFolder' end
  def dscmeta_module_name; 'DFSDsc' end
  ...
  newparam(:dsc_dfsnpath) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "DfsnPath - The DFS Namespace Path to this Replication Group folder is mapped to. This does NOT create the Namespace folders, it only sets the name in the folder object."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
...
Puppet::Type.type(:dsc_dfsreplicationgroupfolder).provide :powershell, :parent => Puppet::Type.type(:base_dsc).provider(:powershell) do
