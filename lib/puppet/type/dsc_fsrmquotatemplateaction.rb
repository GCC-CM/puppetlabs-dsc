...
Puppet::Type.newtype(:dsc_fsrmquotatemplateaction) do
  require Pathname.new(__FILE__).dirname + '../../' + 'puppet/type/base_dsc'
  require Pathname.new(__FILE__).dirname + '../../puppet_x/puppetlabs/dsc_type_helpers'


  @doc = %q{
    The DSC FSRMQuotaTemplateAction resource type.
    Automatically generated from
    'FSRMDsc/DSCResources/DSR_FSRMQuotaTemplateAction/DSR_FSRMQuotaTemplateAction.schema.mof'
  ...
  validate do
      ...
      fail('dsc_percentage is a required attribute') if self[:dsc_percentage].nil?
      fail('dsc_type is a required attribute') if self[:dsc_type].nil?
    end

  def dscmeta_resource_friendly_name; 'FSRMQuotaTemplateAction' end
  def dscmeta_resource_name; 'DSR_FSRMQuotaTemplateAction' end
  ...
  newparam(:dsc_name) do
    ...
    desc "Name - The name of the FSRM Quota Template that this Action is attached to."
    isrequired
    validate do |value|
  ...
  # IsMandatory:  True
  # Values:       None
  newparam(:dsc_percentage) do
    def mof_type; 'uint32' end
    def mof_is_embedded?; false end
    desc "Percentage - This is the threshold percentage the action is attached to."
    isrequired
    validate do |value|
...
Puppet::Type.type(:dsc_fsrmquotatemplateaction).provide :powershell, :parent => Puppet::Type.type(:base_dsc).provider(:powershell) do
